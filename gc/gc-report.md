...
"边"听着像个新概念，其实不是——它就是你已经认识的**引用**，只是把整个堆画成一张图时换了个名字。堆里并不存在一个叫 edge 的结构体：外层数组的第 0 个元素槽里存着内层数组的句柄，**这件事本身**就是一条"外层 → 内层"的边。
...
- 数组 → 每个元素是一条边
- 哈希 → 每个值是一条边
- 闭包 → 指向的函数、捕获的每个自由变量，各是一条边
- class 实例 → 每个字段是一条边（第 1.4 节用 `this.next = other` 造环，靠的就是新增这条边）
...
还有个容易忽略的点：边数的是"份数"，不是"认识不认识"。若数组 `a` 的两个元素都是 `b`，那就是**两条** a → b 的边，`b` 的计数里 `a` 贡献 2。
