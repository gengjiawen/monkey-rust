...
教学路径在同一个信封里继续加料：报告额外序列化 `objectDecisions`（逐对象的 RC 公式与三阶段判定）、typed `visitedEdges`（阶段 1 实际减掉的每条堆内边，带 `fields["next"]` 这样的结构化关系）、deterministic `restorationWitnesses`（平反对象的可达性证明——是证明，不是真实遍历的时间线），以及 `globalRoots`（GC 前一刻「全局变量名 → 对象」的快照，上一节说的那个唯一例外）。Playground 的 **Object decision walkthrough** 用这几样把上面的手算表变成可交互的因果链：被全局变量引用的对象直接标上变量名，visitedEdges 还会画成一张 mermaid 堆拓扑图——实线是阶段 1 减掉的堆内边，一个虚线的 "External refs" 伪节点按 trial RC 标出每个幸存者剩下的非堆引用。
