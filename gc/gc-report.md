...
"边"听着像个新概念，其实不是——它就是你已经认识的**引用**，只是把整个堆画成一张图时换了个名字。堆里并不存在一个叫 edge 的结构体：外层 `Array` 的第 0 个元素槽里存着内层 `Array` 的句柄，**这件事本身**就是一条"外层 → 内层"的边。

沿用柜子的类比：打开 3 号柜，发现里面的包裹上抄着 7 号柜的取件码——这就是一条 3 → 7 的边。而你手机备忘录里记的取件码不在任何柜子里，它是你进入这排柜子的入口——下一节管它叫**根**。
...
堆内边落到 Monkey 里，就藏在各种值的字段里——哪个字段里存着 `GcRef`，哪里就有一条边。常见的有：

- `Array` → 每个元素是一条边
- `Hash` → 每个值是一条边
- `Closure` → 指向的函数、捕获的每个自由变量，各是一条边
- `Instance` → 每个字段是一条边（第 1.4 节用 `this.next = other` 造环，靠的就是新增这条边）

完整的"每种值有哪些边"对照表见第 9.1 节。

#### 边是有方向的：出边与入边

同一根箭头 A → B，站的位置不同，叫法不同：

- 站在 A 的角度，它是 A 的**出边**："我指着谁"。
- 站在 B 的角度，它是 B 的**入边**："谁指着我"。

后文反复出现的两个机制，其实就是同一根箭头的两头：`ref_count`（第 1.4 节）数的是入边条数——含来自根的持有；`trace`（第 4 章）报告的是出边。

还有个容易忽略的点：边数的是"份数"，不是"认识不认识"。若 `Array` `a` 的两个元素都是 `b`，那就是**两条** a → b 的边，`b` 的计数里 `a` 贡献 2。
...
用第 1.1 节的话再说一遍：`ref_count` 就是指向这个对象的边数（堆内入边 + 来自根的边；这个等式会被第 6 章的 trial deletion 临时打破、随后恢复），`dup` 是新增一条指向它的边，`free` 是拆掉一条。**引用是写代码时的说法，边是画图和算账时的说法**——同一个东西。
...
| **出边 / 入边**             | 同一条边的两个视角：A → B 是 A 的出边、B 的入边；`trace` 报出边，`ref_count` 数入边 |
...
能这样做，是因为内部持有有对称结构——第 1.1 节说过，出边和入边是同一根箭头的两头：
...
第 1.1 节的"边"到这里落到实处：所谓出边，就是一个 `Value` 的哪些字段里存着别人的 `GcRef`。逐个变体数一遍：
...
| `Builtin`                                       | 无                        |
| `Class`                                         | `constructor` + 每个方法  |
| `Instance`                                      | `class` + 每个字段        |
| `BoundMethod`                                   | `receiver` + `method`     |

`Instance` 的字段边就是第 1.4 节那个 class 环的原料：`a.connect(b)` 执行 `this.next = other`，等于往图上加一条 a → b 的边。
...
- **label 只说事实，不猜别名。**`Class(Node)#7` 用 class 自己的名字，`Instance(Node)#12` 借 class 的名字，`Closure(makeCycle)#10` 用编译期存进 `CompiledFunction` 的函数名（匿名函数只剩 `Closure#18`），bound method 是 `BoundMethod(Node.connect)#15`。报告不知道那两个 instance 曾经叫 `a` 和 `b`——变量名编译后就不在堆对象身上了，一个对象可能同时有三个别名，也可能一个都没有，猜了就是骗人。唯一的例外是全局变量：编译器的符号表记得每个全局槽位叫什么，`collect_garbage` 动手前把「全局名 → 对象」快照成 `global_roots` 放进报告——这仍是事实而非猜测，因为全局槽位本身就是一条被命名的根引用。上例里它恰好两条：`Node` 和 `makeCycle`；`a`、`b` 是 `makeCycle` 的局部变量，GC 开跑时早已出栈，自然不在其中。
...
教学路径在同一个信封里继续加料：报告额外序列化 `objectDecisions`（逐对象的 RC 公式与三阶段判定）、typed `visitedEdges`（阶段 1 实际减掉的每条堆内边，带 `fields["next"]` 这样的结构化关系）、deterministic `restorationWitnesses`（平反对象的可达性证明——是证明，不是真实遍历的时间线），以及 `globalRoots`（GC 前一刻「全局变量名 → 对象」的快照，上一节说的那个唯一例外；最多 500 条，超出计入 `omittedGlobalRoots`，且每条已报告的根都保证能在对象 catalog 里解析——哪怕逐对象明细被各自的预算截断，被命名对象的摘要也会补进 catalog，命名槽位是非堆引用，对象必然幸存）。Playground 的 **Object decision walkthrough** 用这几样把上面的手算表变成可交互的因果链：被全局变量引用的对象直接标上变量名，visitedEdges 还会画成一张 mermaid 堆拓扑图——实线是阶段 1 减掉的堆内边，一个虚线的 "External refs" 伪节点按 trial RC 标出每个幸存者剩下的非堆引用。
