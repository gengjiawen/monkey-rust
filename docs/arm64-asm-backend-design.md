...
> playground 集成（§12 V1，godbolt 式 ARM64 tab）与 macOS arm64 原生支持
> （`--platform macos`：Mach-O 方言、darwin runtime/链接、Apple Silicon E2E 与 CI，见 §9）也已实施。
> 尚未实施的部分：wasm 单步模拟器（§12 V2）、GC 接入（§14）。
>
> 核心结论：仿照《Compiling to Assembly from Scratch》（Keleshev）的路线，从 AST 单遍 lower 并延迟拼装
> **AArch64 汇编文本**（Linux GNU as/ELF 与 Darwin/Mach-O 两种拼写，指令流一致），
> 交给平台工具链汇编链接成可执行文件——AOT，不做 JIT。
...
本提案补上第四条：AOT-to-native；当前唯一目标是 Linux AArch64，不是泛化的 arm64 平台后端。
...
- `monkey-asm build foo.monkey -o foo`：生成 `foo.s`，调用交叉工具链汇编链接出
  静态 Linux AArch64 ELF；`monkey-asm run` 仅在 Linux AArch64 上直接执行，在其他主机上
  透过 `qemu-aarch64` 执行。
...
- 汇编方言只覆盖两种 arm64 拼写：Linux GNU as/ELF 与 Darwin/Mach-O（CLI 以
  `--platform linux|macos` 选择，默认随 host，见 §9）；不支持 Windows。
- 不支持 x86_64 目标。
...
- CLI 定位为**仓库内开发工具**：未设置 `MONKEY_ASM_RUNTIME` 时，每次 `build`/`run` 都调用 cargo
  对精确的 release cross target 做 freshness check，依赖仓库源码在场；显式 override 才跳过该步骤。
...
#    （--platform macos 时 target 换成 aarch64-apple-darwin）
```

`build` 子命令串起链路：生成 `out.s` → 未显式 override 时在仓库内调用 cargo 构建/检查最新的
release `libmonkey_asm.a`（见 §2.2 的 CLI 定位）→

```bash
aarch64-linux-gnu-gcc out.s libmonkey_asm.a -o prog -static -lpthread -ldl -lm -lrt -lutil
qemu-aarch64 ./prog
```

上面列的是 linux-gnu fallback；CLI 读取 rustc 为该 target 报出的 `native-static-libs` 并据此组装
最终链接参数，不能假定不同 Rust/toolchain 版本的依赖集合恒定。静态链接时过滤 rustc 报出的
`-lgcc_s`：常见交叉工具链没有静态 `libgcc_s`，GCC driver 会自行加入静态 `libgcc/libgcc_eh`。
runtime 路径遵守 `CARGO_TARGET_DIR`。`--platform macos` 的对应链路是
`cc out.s libmonkey_asm.a -o prog -arch arm64`（fallback libs `-lSystem -lc -lm`；macOS 没有
静态 libSystem，因此不加 `-static`），产物在 Apple Silicon 上直接原生执行。
...
变参走寄存器；Darwin 变参走栈，所以这个临时通道没有 Darwin 版本）。阶段二起 `printf`
...
- **具名函数的自引用**：lower 进入函数作用域后，若 `FunctionDeclaration.name` 非空
  ...
  普通字节码编译器与 AOT lower 都执行这一步，并由局部递归回归测试固定，见 §13。
...
- native `PointerStore` 把显式 `#[repr(align(8))]` 的 `HeapCell` 保存在
  `HashMap<Value, Box<HeapCell>>`，把 box 内 cell 的稳定地址编码成低位 `001`；解码除检查 tag 外，
  还必须确认完整 Value 是当前 store 的 key，伪造地址和其他 store 的 Value 都不会被解引用；
- native FFI 使用一个进程级 `OnceLock<Mutex<PointerStore>>`，值可跨 FFI 调用存活；生成代码回调前
  释放 store guard，回调中的 `rt_*` 才能重新取锁，store 不会在每个入口被重新创建；
...
- 内存策略：v1 由进程级 store 显式持有所有 cell，运行中不逐对象回收，进程退出时由操作系统回收；
  不借助泄漏 API，也不做任意裸地址解引用。第 8 章可选换成 `gc` crate + 精确栈扫描。
- 已知坑：**阶段二起不得从汇编调 `printf`**（变参 ABI 平台差异，Darwin 走栈）；
  阶段一的 `printf` 用法是 Linux-only 的临时通道（§5.1），阶段二删除。

## 9. 平台与工具链

- 汇编方言由 `emitter::AsmDialect` 参数化，两种拼写共享同一条指令流：
  - **Linux GNU as/ELF**：C 符号无前缀（`main`、`bl rt_add`）、`adrp`+`:lo12:` 重定位、
    `.section .rodata` 与 `.bss`+`.balign`+`.skip`、`.L` 私有 label。
  - **Darwin/Mach-O**：C 符号带 `_` 前缀（`_main`、`bl _rt_add`；darwin staticlib 导出的
    正是 `_rt_*`）、`adrp`+`@PAGE`/`@PAGEOFF`、`.section __TEXT,__const` 与
    `.zerofill __DATA,__bss,g_globals,size,3`（对齐参数是 log2；0 个全局也保留 8 字节）、
    `L` 私有 label。`g_globals` 是模块内部符号，两边都不带前缀。
- CLI 以 `--platform linux|macos` 选择（默认随 host）：runtime target 对应
  `aarch64-unknown-linux-gnu` / `aarch64-apple-darwin`，链接器默认 `aarch64-linux-gnu-gcc`
  （`-static`）/ `cc`（`-arch arm64`；macOS 没有静态 libSystem，不加 `-static`）。
  `emit` 在任何 host 都可用；`--platform macos` 的**链接**需要 macOS host（Apple SDK，
  设 `MONKEY_ASM_CC` 可覆盖），**执行**需要 Apple Silicon——qemu-user 只模拟 Linux ELF，
  没有 Mach-O 等价物。
- x86_64 开发机全链路可跑 Linux 口味：`aarch64-linux-gnu-gcc` 交叉汇编链接 + `qemu-aarch64`
  用户态执行（原书 QEMU 附录的同款流程）。任何阶段都不能用 host 原生 `cc` 直接汇编另一平台的
  arm64 文本。
- CI 三个相关 job：x86_64 Linux 常规 job 跑单元/`.s` 快照；x86_64 Linux 专用 job 装交叉工具链、
  Rust target 与 QEMU，强制跑 ABI/端到端集合；`macos-15`（Apple Silicon）job 原生强制跑同一套
  端到端集合的 Mach-O 口味（darwin 探针 + 全部程序语料，`MONKEY_ASM_E2E_REQUIRED=1`）。
- playground/wasm 恒定使用 Linux/ELF 拼写（§12），不随浏览器所在平台变化。
...
| 8（可选） | 对接 `gc` crate（精确栈扫描 root）替换进程生命周期持有策略                                                                                                                                                                                                    | 真 GC                                        | 另计     |
...
native Linux AArch64 可用 CLI 真机验证；当前自动化由 x86_64 Linux + QEMU CI 承担，playground 负责"看懂"。

## 13. 已修复的作用域前置问题

旧实现进入具名函数作用域后没有调用 `define_function_name`，局部的
`let f = fn() { f() }` 会错误地捕获尚未初始化的外层 local。当前普通字节码编译器和 AOT lower
都会在函数作用域定义 `SymbolScope::Function`：普通 VM/gc VM 通过 `OpCurrentClosure` 读取当前闭包，
AOT 通过 spill 后的闭包槽读取，局部递归不再依赖外层绑定是否已写入。

`let` 的另一个相关规则也已统一：先编译/降低 RHS，再定义同名的新 slot。因此
