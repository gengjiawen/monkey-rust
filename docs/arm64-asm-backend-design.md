...
> **Linux AArch64 的 GNU 汇编文本**，交给交叉工具链汇编链接成静态 ELF——AOT，不做 JIT。
...
- 只支持 Linux GNU as 方言和 ELF 链接；当前不生成 Darwin 汇编或 Mach-O，Apple Silicon macOS
  不能原生执行产物。原生 macOS 支持需要独立的平台适配，见 §9/§14。
- 不支持 x86_64 目标。
...
- 当前 CI 全部运行在 x86_64 Linux：常规 job 跑单元/`.s` 快照，专用 job 安装交叉工具链、Rust target
  与 QEMU，强制跑 ABI/端到端集合；当前没有原生 ARM runner job。
- macOS/Apple Silicon：当前 CLI 仍生成 Linux ELF，因此不会误判为可原生运行，而是走 QEMU 路径。
  真正原生支持是完整平台适配：还需要 Darwin 符号/section/重定位方言、
  `aarch64-apple-darwin` runtime、clang/Mach-O 链接参数与 native libs、平台选择及原生 E2E。
...
- Darwin/Mach-O 平台后端：补齐符号、section、重定位、runtime target、clang 链接与原生 E2E，
  让 Apple Silicon 可直接运行；它是明确的平台适配，不只是改符号前缀。
