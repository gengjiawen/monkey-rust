...
> 状态：Implemented
>
> 范围：产品、交互、diagnostics 与测试方案；实现已落地于 `gc/` diagnostics、`wasm` envelope 与 Playground walkthrough UI。
...
全局根同样受 §13 的预算约束：最多 `MAX_GLOBAL_ROOTS = 500` 条，超出部分计入顶层 `omittedGlobalRoots`，不能静默截断。保留优先级：对象已在 catalog 里的根优先——名字要标在读者看得见的行与节点上才有意义——同一优先级内维持槽位顺序。

不变量：每条已报告根的 `objectId` 必须能在 catalog 中解析。phase 预算挑选 catalog 时不看根集合，保留下来的根可能命名了被 decision 截断丢掉的对象，此时 collector 把该对象的 summary 补进 catalog——被命名的全局槽位是非堆引用，对象必然幸存于本次 collection，补录总是安全的。此外 `name` 不得重复；被命名的对象若有 decision 记录，decision 必为 `survivor`——命名槽位是非堆引用，trial deletion 减不掉它，减得掉就是 collector 的 bug。

## 10. 运行时采集流程

只增强 `run_gc_with_stats()` 的只读 diagnostics 路径，普通 `run_gc()` 不生成 label、边详情或 witness，避免为正常 collector 引入不必要开销。
...
- 若让 `trace` 复用 `visit_edges`，每次普通 GC 都要为 diagnostics 的排序付出分配开销，得不偿失。
...
聚合指标必须始终完整；详细 diagnostics 必须有明确上限，避免大型源码产生不可控 WASM JSON 和 DOM。
...
- `globalRoots` 的 `objectId` 能在 catalog 中解析（§9.5 的补录不变量保证 collector 侧成立，parser 无条件强制）、`name` 不重复、`omittedGlobalRoots` 是非负 safe integer；被命名对象若有 decision 记录则必为 `survivor`（§9.5）——decision 被截断时该项跳过，不误报。
...
### Phase A：Trial decision diagnostics
...
- 普通 `run_gc()` 行为和开销模型不因教学 diagnostics 改变；
