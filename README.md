...
- **Debugger** (`debugger` statement): a JavaScript-style `debugger` statement that stays transparent in every backend (interpreter, both VMs, AOT asm, prettier/minifier/linter); on the playground's **Debugger** tab each hit records a snapshot on the GC VM — the call stack with named locals, captures and globals beside a budgeted Mermaid heap graph, with `ref #n` chips linking slots to the heap objects they point at ([design doc](docs/playground-debugger-plan.md)).
