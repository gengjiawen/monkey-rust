 ...
 * workspace package still names a different version — that is the shape the
 * omission takes, and catching it here costs a re-run, while catching it after
 * publish means a release whose packages depend on the previous release.
 */
function assertWorkspaceDependenciesAreSynced(version: string) {
  const packagesDir = repoPath('packages')
  const manifests: { directory: string; packageJson: PackageJson }[] = []
  // `wasm/pkg` is generated by wasm-pack from the crate version, so it is a
  // dependency target but never a manifest of ours to check.
  const workspaceNames = new Set<string>(['@gengjiawen/monkey-wasm'])
  ...
  for (const entry of readdirSync(packagesDir)) {
    ...
    manifests.push({ directory: entry, packageJson })
  }

  const stale: string[] = []
  for (const { directory, packageJson } of manifests) {
    for (const field of DEPENDENCY_FIELDS) {
      const ranges = (packageJson[field] ?? {}) as Record<string, string>
      for (const [dependency, range] of Object.entries(ranges)) {
        if (!workspaceNames.has(dependency)) {
          continue
        }
        // A range with no version number in it (`workspace:*`, `*`, `latest`)
        // resolves to whatever the workspace holds and cannot go stale.
        if (!/\d/.test(range)) {
          continue
        }
  ...
  if (stale.length > 0) {
    throw new Error(
      `These workspace dependencies were not bumped to ${version}:\n  ` +
        stale.join('\n  ') +
        '\nAdd the package to the sync blocks in this script; releasing as is ' +
        'would ship packages that depend on the previous version.'
    )
  }
}

assertWorkspaceDependenciesAreSynced(nextVersion)
